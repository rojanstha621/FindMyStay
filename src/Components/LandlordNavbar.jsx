...
function LoggedInNavbarLandlord() {
  const [isOpen, setIsOpen] = useState(false);

  const toggleMenu = () => setIsOpen(!isOpen);

  return (
    <nav className="bg-[#FFFFF] text-[#594E4E] font-body shadow-md">
      <div className="max-w-7xl mx-auto px-0 py-6 flex justify-between items-center">
        ...
        <ul className={`md:flex md:space-x-8 font-semibold text-[15px] absolute md:static top-16 left-0 w-full md:w-auto px-6 md:px-0 ${isOpen ? 'block' : 'hidden'}`}>
          <li className="py-2 md:py-0">
            <Link to="/landlordDashboard" className={({ isActive }) => `block hover:text-black ${isActive ? 'text-[#594E4E]' : ''}`}>
              🏠 Dashboard
            </Link>
          </li>
         
          <li className="py-2 md:py-0">
            <Link to="/landlord/profile" className={({ isActive }) => `block hover:text-black ${isActive ? 'text-[#594E4E]' : ''}`}>
              👤 Profile
            </Link>
          </li>
          <li className="py-2 md:py-0">
...
export default LoggedInNavbarLandlord;
